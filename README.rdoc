...
Note that, setup.rb will not work 100% as an install option as it will
cause the optional extensions to be loaded automatically along with the
default core extensions when using `require "facets"` or `require "facets/{name}"`.

However, if that is not an issue, or if cherry picking only, then Facets
can still be installed manually with Ruby Setup (http://rubyworks.github.com/setup).
