...
Facets 2.7.0 is the biggest release of Facets since 2.4. Rather then trickle-release these
changes of the course of the 2.6.x series, 2.7 handles them all at once. This release nearly
completes the process of trimming down or the MORE library. Over 30 high-level libraries have
been spun-off as separate gems and over 10 other libraries have been deprecatedf for various
resaons. A few other major changes have been made that may effect your code. In particular
#class_extension has been renamed to #class_extend (require 'facets/class_extend').
...
available via Facets too. Ultimately some these will be removed
from Facets as well (most noteably the ansicode libraries).

    LIBRARY               GEM
    --------------------  ------------------------------
    ansicode.rb           ansicode -or- clio
    progressbar.rb        ansicode -or- clio
    logger.rb             ansicode (partially)
...
The libraries have been deprecated but do not yet have a replacement.
...
These libraries have simply been deprecated becuase they were found lacking
in some significant fashion, or otherwise made available elsewhere.

    * 1stclassmethod.rb   #method! and #instance_method! are now part of core.
    * attr.rb             Added Module#attr_setter to core, and separated out the rest.
    * coroutine.rb        Depended on continuations, which are not supported in 1.9+.
    * nilstatus.rb        It proved rather useless, not to mention trivial.
    * heap.rb             Heap was just an alias for PQueue anyway. Use 'pqueue' instead.
    * dependency.rb       Other solutions exist that are much better.
    * classmethods.rb     #class_extend solution is more robust.
    * ziputils.rb         Have a look at Folio (gem install folio) for replacement.
    * consoleutils.rb     #ask is in core, for the rest see Clio project.
    * minitar.rb          Available as a gem and in Folio.
    * elementor.rb        #per has been added to core. 
    * elementwise.rb      #ewise has been added to core. 
    * unheritable.rb      It's implementation is trivial and usefulness questionaable.
